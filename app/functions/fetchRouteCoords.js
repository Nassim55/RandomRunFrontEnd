      ...
      if (isLocationPermissionGranted === true) {
        ...
        if (token) {
          const response = await fetch(
            `${HOST_URL}/route/getroute?longitude=${originLongitude}&latitude=${originLatitude}&routeDistance=${routeDistanceMeters}`,
            {
              method: 'GET',
              headers: {
          ...
          if (data.coordinates) {
            ...
            dispatch(
              setMostNorthEasternCoordinates(data.mostNorthEastCoordinates),
            );
            dispatch(
              setMostSouthWesternCoordinates(data.mostSouthWestCoordinates),
            );
          } else if (data.detail) {
            const throttleTime = new Date(data.detail.replace(/\D/g, '') * 1000)
              .toUTCString()
              .split(' ')[4];
            Alert.alert(
              'Route Not Generated',
