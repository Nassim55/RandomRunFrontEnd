...
const saveRoute = async (
  ...
  try {
    ...
    routeCoordinates = JSON.stringify(routeCoordinates)
      .replace(/\[/g, '{')
      .replace(/\]/g, '}');
    mostNorthEasternCoordinates = JSON.stringify(mostNorthEasternCoordinates)
      .replace(/\[/g, '{')
      .replace(/\]/g, '}');
    mostSouthWesternCoordinates = JSON.stringify(mostSouthWesternCoordinates)
      .replace(/\[/g, '{')
      .replace(/\]/g, '}');
    ...
    if (
      data.response ===
      'Route not saved. You have exceeded your limit of five routes'
    ) {
      Alert.alert(
        'Not Saved',
