...
const SavedRouteCards = (props) => {
  ...
  const cards = savedRoutesResponse
        ...
        {cards.length > 0 ? (
              ...
              currentIndex < index * step + step && (
                <Card
                  ...
                  onPress={() => {
                    ...
                    const coordinatesDecimal = coordinates.map(
                      (coordsSet, index) =>
                        coordsSet.map((coord) => parseFloat(coord)),
                    );
                    const mostNorthEasternCoordinatesDecimal = mostNorthEasternCoordinates.map(
                      (element) => parseFloat(element),
                    );
                    const mostSouthWesternCoordinatesDecimal = mostSouthWesternCoordinates.map(
                      (element) => parseFloat(element),
                    );
                    ...
                    dispatch(
                      setFinalRouteLineString({
                        type: 'LineString',
                        coordinates: coordinatesDecimal,
                      }),
                    );
                    dispatch(
                      setMostNorthEasternCoordinates(
                        mostNorthEasternCoordinatesDecimal,
                      ),
                    );
                    dispatch(
                      setMostSouthWesternCoordinates(
                        mostSouthWesternCoordinatesDecimal,
                      ),
                    );
          ...
          <View style={styles.textContainer}>
            <Text style={styles.text}>
              You don't currently have any saved routes. You're saved route
      ...
      <View style={styles.footerContainer}>
        ...
        <View style={styles.buttonContainerSmall}>
          <Pressable
            style={({pressed}) => [
              styles.buttonCircle,
              styles.buttonCircleSmall,
              {
                opacity: pressed ? 0.5 : 1,
                backgroundColor: pressed ? '#f7f6f2' : 'white',
              },
            ]}
